CREATE OR REPLACE FUNCTION public.v2_request_supply_reconfirmation(
  _request_id uuid,
  _supply_id uuid,
...
BEGIN
  ...
  INSERT INTO public.v2_reconfirmation_tasks (
    ...
    CASE WHEN _agent IS NULL THEN 'open' ELSE 'assigned' END,
    auth.uid()
  )
...
REVOKE ALL ON FUNCTION public.v2_request_supply_reconfirmation(uuid, uuid, text, text, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.v2_request_supply_reconfirmation(uuid, uuid, text, text, date) TO authenticated, service_role;

-- Requests reference their pending reconfirmation tasks for the UI badge (privacy-safe: no supplier identity).
CREATE OR REPLACE FUNCTION public.v2_sourcing_request_tasks(_request_id uuid)
RETURNS TABLE(task_id uuid, supply_id uuid, status text, priority text, due_date date, created_at timestamptz)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
  SELECT t.id, t.supply_id, t.status::text, t.priority, t.due_date, t.created_at
  FROM public.v2_reconfirmation_tasks t
  JOIN public.v2_sourcing_requests r ON r.id = t.sourcing_request_id
  WHERE t.sourcing_request_id = _request_id
    AND (public.v2_is_org_member(r.organization_id, auth.uid()) OR public.v2_is_agrigrid_admin(auth.uid()))
  ORDER BY t.created_at DESC
$function$;

REVOKE ALL ON FUNCTION public.v2_sourcing_request_tasks(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.v2_sourcing_request_tasks(uuid) TO authenticated, service_role;